...
async function walk({ from, to, civilianId }) {
  ...
  try {
    ...
    if (coordinates && coordinates.length > 1) {
      ...
      // Frontend-friendly movement with smooth updates - IMPROVED PATH FOLLOWING
      // Use all waypoints for accurate path following, but limit update frequency for performance
      ...
      if (allCoordinates > maxUpdateRate) {
        ...
        // For very short routes, interpolate between waypoints
        totalUpdates = minUpdateRate;
        skipRate = -1; // Signal to use interpolation
      }
      ...
      if (skipRate === -1) {
        ...
        for (let i = 1; i < totalUpdates; i++) {
          ...
          const currentLocation = {
            latitude: lowerCoord[1] + (upperCoord[1] - lowerCoord[1]) * interpolationFactor,
            longitude: lowerCoord[0] + (upperCoord[0] - lowerCoord[0]) * interpolationFactor
          };
          ...
          civilianAbility.publishEvent('civilian.location_update', {
            ...
            waypoint: `interpolated_${lowerIndex}_${upperIndex}`
          });
          
          console.log(`📍 ${civilianId} interpolated location: ${currentLocation.latitude.toFixed(6)}, ${currentLocation.longitude.toFixed(6)} (${(progress*100).toFixed(1)}% complete)`);
          
          await new Promise(resolve => setTimeout(resolve, realUpdateIntervalMs));
        }
      } else {
        // Standard mode: use actual waypoints with skipping
    ...
    const maxUpdates = 100; // Significantly more updates for smoother fallback
    ...
    for (let i = 1; i <= maxUpdates; i++) {
      ...
      // More sophisticated interpolation - could add some path deviation here
      const currentLat = from.latitude + (latDiff * progress);
      const currentLng = from.longitude + (lonDiff * progress);
      
      // Optional: Add slight random deviation to simulate real walking (uncomment if desired)
      // const deviation = 0.00001; // Small deviation
      // const randomLat = currentLat + (Math.random() - 0.5) * deviation;
      // const randomLng = currentLng + (Math.random() - 0.5) * deviation;
      
      const currentLocation = {
        latitude: currentLat, // or randomLat for deviation
        longitude: currentLng  // or randomLng for deviation
      };
